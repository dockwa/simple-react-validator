class SimpleReactValidator{
  constructor(){
    ...
    this.rules = {
      ...
      'gte'            : {message: 'The :attribute must be greater than or equal to :gt.',          rule: (val, options) => this._testRegex(val,/^\d+.?\d*$/) ? parseFloat(val) >= parseFloat(options[0]) : false, messageReplace: (message, options) => message.replace(':gte', options[0]) },
      ...
      'lt'             : {message: 'The :attribute must be less than :gt.',                         rule: (val, options) => this._testRegex(val,/^\d+.?\d*$/) ? parseFloat(val) < parseFloat(options[0]) : false, messageReplace: (message, options) => message.replace(':lt', options[0]) },
      'lte'            : {message: 'The :attribute must be less than or equal to :gt.',             rule: (val, options) => this._testRegex(val,/^\d+.?\d*$/) ? parseFloat(val) <= parseFloat(options[0]) : false, messageReplace: (message, options) => message.replace(':lte', options[0]) },
